...
class CapitalSession:
    ...
    def get_headers(self):
        with self.lock:
            if time.time() < self.expires and self.cst:                return {
...
def get_candles(epic, resolution, max_candles=30):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            closes, highs, lows = [], [], []            for p in prices_data:
...
def pyramiding_kontrol(h, epic, instrument):
    ...
    for p in epic_pozisyonlar:
        upl = float(p['position']['upl'])
        level = float(p['position']['level'])        size = float(p['position']['size'])
...
KRITIK KURALLAR:
1. TEKNİK FİLTRE: En az 2/3 indikatör (MA+ADX+RSI) onay vermeli2. SPREAD: Maksimum 0.5 spread - yüksek spread'te işlem YOK
...
KOMUT: Teknik sinyalleri değerlendir (2-of-3 kuralı), spread kontrol et, Gremium oylama yap, uygun ise trade üret."""
    ...
    for attempt in range(total_attempts):        current_model = get_next_model()
        ...
        try:            dongu_sayaci += 1
            ...
            if dongu_sayaci % 6 == 0:
                ...
                for k, v in MARKET_CONFIG.items():
                    ...
                    if stufe > 0:                        ozet += f"• {k}: Seviye {stufe}/4\n"
                if "Seviye" in ozet:
